...
export async function sendOtpEmail(toEmail: string, otp: string): Promise<void> {
  ...
  try {
    await transport.sendMail({
      ...
      to: toEmail,
      subject: 'Your Hive Activation OTP',
      text: `Your OTP for Hive account activation is: ${otp}\n\nThis OTP expires in ${config.activation.otpExpiryMinutes} minutes.`,
      html: `<p>Your OTP for Hive account activation is: <strong>${otp}</strong></p><p>This OTP expires in ${config.activation.otpExpiryMinutes} minutes.</p>`,
    });
